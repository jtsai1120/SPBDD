...
# CUDD ships a configure generated by automake 1.14, and git gives every
# checked-out file the same timestamp -- so make cannot tell that the generated
# files are current, and tries to rebuild them with an automake no current
# distribution has. Nothing actually needs regenerating, so say so rather than
# depend on a decade-old autotools.
	$(MAKE) -C $(CUDD_DIR) -j$(NPROC) ACLOCAL=: AUTOCONF=: AUTOMAKE=: AUTOHEADER=:
